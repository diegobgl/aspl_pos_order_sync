...
class PosOrder(models.Model):
    ...
    def unlink(self):
        cashier = self.env['res.users'].search([('sales_persons', 'in', [self.salesman_id.id])])
        # cashier.append(self.salesman_id.id)
        notifications = []
        for user in cashier:
            session = self.env['pos.session'].search([('user_id', '=', user.id)], limit=1)
            if session:
                notif = {'cancelled_order': self.read()}
                notifications.append([user.partner_id, 'order.sync', notif])
