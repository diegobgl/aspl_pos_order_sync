odoo.define('aspl_pos_order_sync.chrome', function (require) {
    'use strict';
    ...
    const ChromeInherit = (Chrome) => class extends Chrome {
        ...
        _onNotification(notifications){
            ...
            for(let notif of notifications) {
                ...
                var notif_data = notif.payload
                if (notif_data && notif_data.cancelled_order){
                    previous_orders = previous_orders.filter(function(obj){
                        return obj.id !== notif_data.cancelled_order[0].id;
                    });
                    ...
                    this.render();
                }else if(notif_data && notif_data.new_pos_order){
                    previous_orders.push(notif_data.new_pos_order[0]);
                    var obj = {};
